...
%% Calculate T matrix
T_HbO_brain = squeeze(dA(:,1:2,1)); %4146*2
T_HbR_brain = squeeze(dA(:,1:2,2)); %4146*2
...
figure('name','scalp')
T_scalp = atlasViewer.fwmodel.mesh_scalp.faces;
trimesh(T_scalp, scalp_vertices(:,1),scalp_vertices(:,2),scalp_vertices(:,3))
hold on
...
trisurf(T,brain_vertices(:,1),brain_vertices(:,2),brain_vertices(:,3),intensity,'edgecolor','none')
mask = find(intensity>-2);
fprintf('length is %d\n', length(mask))
figure
intensity_scalp = log10(sum(Adot_scalp(:,:,1),1));
trisurf(T_scalp,scalp_vertices(:,1),scalp_vertices(:,2),scalp_vertices(:,3),intensity_scalp,'edgecolor','none')
mask_scalp = find(intensity_scalp>-2);
fprintf('scalp length is %d\n', length(mask_scalp))

Adot_mask = Adot(:, mask, :);
Adot_scalp_mask = Adot_scalp(:, mask, :);
...
% Amatrix_brain_masked = 
%% my original code

% n_g = size(G,1);
% n_t = size(T_HbO_brain,2);
% H_brain = zeros(n_channels*time_points,n_g*n_t); % H_brain = [414600, 844]
% j = 1;
% for i_g = 1:n_g
%     fprintf('%d/%d\n',i_g,n_g)
%     for i_t = 1:n_t
%         HbO = kron(G(i_g,:),T_HbO_brain(:,i_t)); % HbO = [4146, 20004]% G 422,20004
%         HbR = kron(G(i_g,:),T_HbR_brain(:,i_t)); % HbR = [4146, 20004]
%         % T_HbO_brain 4146,2
%         % T_HbR_brain 4146,2
%         
%         gt = [HbO,HbR]'; % gt = [40008, 4146]
%         Agt = Amatrix_brain*gt; % Amatrix = [100, 40008], Agt = [100, 4146]
...
HTH = zeros(size(G,1)*size(T_HbO_brain,2)+ size(G_scalp,1)*size(T_scalp,2)); % HTH = [1566, 1566]
HTY = zeros(size(G,1)*size(T_HbO_brain,2)+ size(G_scalp,1)*size(T_scalp,2), 1); % HTY = [1566, 1]
j1 = 0;
...
for i_g = 1:(size(G,1)+ size(G_scalp,1))
    ...
    if i_g <= size(G,1)
        g = G(i_g,:); % G = [422, 20004], g = [1, 20004]
        n_t = size(T_HbO_brain,2);  
        T_HbO = T_HbO_brain; % T_HbO_brain = [4146,2],
        T_HbR = T_HbR_brain; % T_HbR_brain = [4146,2],
        Amatrix = Amatrix_brain; % Amatrix = [100, 40008]
    else
...
%         gt = [HbO,HbR]';
        gt = [kron(g,T_HbO(:,i_t)),kron(g,T_HbR(:,i_t))]'; % gt = [40008, 4146],
        ...
        for i_g2 = 1:(size(G,1)+ size(G_scalp,1))
            if i_g2 <= size(G,1)
                g2 = G(i_g2,:); % g2 = [1, 20004]
                n_t2 = size(T_HbO_brain,2);
                T_HbO2 = T_HbO_brain; % T_HbO2 = [4146, 2]
                T_HbR2 = T_HbR_brain; % T_HbR2 = [4146, 2]
                Amatrix2 = Amatrix_brain; % Amatrix2 = [100, 40008]
            else
...
%                 HbR2 = kron(g2,T_HbR2(:,i_t2)); % HbR2 = [4146, 20004]
                gt2 = [kron(g2,T_HbO2(:,i_t2)),kron(g2,T_HbR2(:,i_t2))]'; % gt2 = [40008, 4146]
...
%ram is 3770M, so used 2.5G ram, disk is not working
%% Ted code
%Group1/2- HbO and HbR in brain
%Group3/4- HbO and HbR in scalp
...
% LO = [Amatrix_brain, Amatrix_scalp];
LO = double(Amatrix_brain);
Y = OD;

R = estimateMeasureNoise(Y');
Q = estimateStateNoise(GO,TO,.001);


alphaR = 1;
alphaQ = 1E-6;

[x C]=StaticEstimator(Y,LO,TO,GO,R,Q,alphaQ,alphaR);
%% do inverse
